...
#' @noRd
app_sys <- function(...) {
  system.file(..., package = "bayesadapt")
}
...
#' Add a resource path safely (no-op if golem is unavailable)
#' @noRd
add_resource_path <- function(prefix, directoryPath, warn_empty = FALSE) {
  if (requireNamespace("golem", quietly = TRUE)) {
    golem::add_resource_path(prefix, directoryPath, warn_empty)
  } else if (dir.exists(directoryPath)) {
